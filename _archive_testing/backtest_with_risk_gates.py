#!/usr/bin/env python3
"""
เปรียบเทียบ 3 ระบบ (v11.2, New V4, Hybrid) บน GC=F 60 วัน
โดยใส่ Risk Gates: Daily DD 3%, Max Consecutive Losses 5
ใช้ Position Sizing 1% Risk, Max 10 Contracts
"""
...
# ── Trade Functions (same as before, return trades with timestamp) ─
def v112_trades(df):
    trades = []
    for i in range(20, len(df)-40):
        ...
        trades.append({'dir':direction,'entry':entry,'exit':exit_price,'sl':sl,'time':row.name})
    return trades
    ...
    for i in range(20, len(df)-40):
        ...
        if (row['EMA20']>row['EMA50'] and row['Swing_H']>row['Swing_L'] and row['Diff']>0):
            ...
            if gl<=row['close']<=gh and row['Bull_Sweep'] and row['low']<=row['BB_Lower']*1.02:
                ...
                trades.append({'dir':'BUY','entry':entry,'exit':exit_price,'sl':sl,'time':row.name})
        if (row['EMA20']<row['EMA50'] and row['Bear_Sweep'] and row['high']>=row['BB_Upper']*0.98):
            ...
            trades.append({'dir':'SELL','entry':entry,'exit':exit_price,'sl':sl,'time':row.name})
    return trades
...
def simulate_with_risk_gates(trades, initial=10000, risk_pct=0.01, max_contracts=10,
    ...
    for t in trades:
        ...
        if trade_day != current_day:
            ...
            consec_losses = 0
            stop_trading_day = False
            stopped_days += 0  # already counted

        if stop_trading_day:
            continue  # skip remaining trades today
...
def stats(curve, trades_result, initial, stopped_days):
    ...
    return {
        'final':final_eq, 'return':ret, 'dd':max_dd, 'wr':wr, 'pf':pf,
        'total_trades':len(trades_result), 'stopped_days':stopped_days
    }

# ── Run All ───────────────────────────────────────
df_15m = add_indicators(df).dropna()
print("Running v11.2 with Risk Gates...")
trades_v112 = v112_trades(df_15m)
res_v, eq_v, stop_v = simulate_with_risk_gates(trades_v112)
s_v = stats(eq_v, res_v, 10000, stop_v)

print("Running New V4 with Risk Gates...")
trades_new = new_v4_trades(df_15m)
res_n, eq_n, stop_n = simulate_with_risk_gates(trades_new)
s_n = stats(eq_n, res_n, 10000, stop_n)

print("Running Hybrid with Risk Gates...")
trades_hyb = hybrid_trades(df_15m)
res_h, eq_h, stop_h = simulate_with_risk_gates(trades_hyb)
s_h = stats(eq_h, res_h, 10000, stop_h)

# ── Output ────────────────────────────────────────
print("\n📊 WITH RISK GATES (Daily DD 3%, Consec Loss 5)")
print("="*90)
