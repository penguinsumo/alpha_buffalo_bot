#!/usr/bin/env python3
"""
backtest_new_v4b.py — เปรียบเทียบ NEW_V2 vs NEW_V4b (Sweep Buffer 0.05% + Confirmation Bar: Low_i+1 > Low_prev)
"""
...
def add_indicators(df, df_1h):
    ...
    # NEW_V4 sweep buffer: หลุด 0.05% แล้วกลับ (close > Low_prev)
...
def run_backtest(df, method):
    ...
    min_bars = 20
    max_bars = 40
    for i in range(min_bars, len(df)-max_bars-1):
        row = df.iloc[i]
        ...
        if row['EMA20'] > row['EMA50']:
            if method == 'NEW_V2':
                ...
                if row['Bull_Sweep_V4']:
                    # Confirmation: แท่งถัดไป (i+1) ต้องมี Low > Low_Prev (ไม่ทำ Lower Low)
                    next_row = df.iloc[i+1]
                    if next_row['low'] > row['Low_Prev']:
            ...
            if method == 'NEW_V2':
                ...
                if row['Bear_Sweep_V4']:
                    next_row = df.iloc[i+1]
                    if next_row['high'] < row['High_Prev']:  # ไม่ทำ Higher High
        ...
        start_j = i+2 if method == 'NEW_V4b' else i+1
        for j in range(start_j, min(i+max_bars, len(df))):
...
if __name__ == "__main__":
    ...
    print("="*100)
    header = f"{'':<20} {'NEW_V2 (GF+Sweep)':<25} {'NEW_V4b (Buf+Conf Low)':<25}"
    print(header)
    print(f"{'Trades':<20} {results['NEW_V2'][0]['total']:<25} {results['NEW_V4b'][0]['total']:<25}")
    print(f"{'Win Rate':<20} {results['NEW_V2'][0]['wr']:.1f}%{'':<21} {results['NEW_V4b'][0]['wr']:.1f}%")
    print(f"{'Total PnL':<20} {results['NEW_V2'][0]['pnl']:+.2f}%{'':<20} {results['NEW_V4b'][0]['pnl']:+.2f}%")
    print(f"{'Max DD':<20} -{results['NEW_V2'][0]['dd']:.2f}%{'':<21} -{results['NEW_V4b'][0]['dd']:.2f}%")

    print("\n📊 SELL SIDE COMPARISON")
    print("="*100)
    header = f"{'':<20} {'NEW_V2 (GF+Sweep)':<25} {'NEW_V4b (Buf+Conf Low)':<25}"
    print(header)
    print(f"{'Trades':<20} {results['NEW_V2'][1]['total']:<25} {results['NEW_V4b'][1]['total']:<25}")
    print(f"{'Win Rate':<20} {results['NEW_V2'][1]['wr']:.1f}%{'':<21} {results['NEW_V4b'][1]['wr']:.1f}%")
    print(f"{'Total PnL':<20} {results['NEW_V2'][1]['pnl']:+.2f}%{'':<20} {results['NEW_V4b'][1]['pnl']:+.2f}%")
    print(f"{'Max DD':<20} -{results['NEW_V2'][1]['dd']:.2f}%{'':<21} -{results['NEW_V4b'][1]['dd']:.2f}%")

    total_v2 = results['NEW_V2'][0]['pnl'] + results['NEW_V2'][1]['pnl']
    total_v4b = results['NEW_V4b'][0]['pnl'] + results['NEW_V4b'][1]['pnl']
    print(f"\n💰 Total PnL: NEW_V2 = {total_v2:+.2f}% | NEW_V4b = {total_v4b:+.2f}%")
