#!/usr/bin/env python3
"""Backtest with 1% Risk Position Sizing (Capped Leverage) — GC=F 15m 60 days"""
...
def v112_trades(df):
    ...
    for i in range(20, len(df)-40):
        ...
        trades.append({'dir':direction,'entry':entry,'exit':exit_price,'sl':sl,'session':get_session(hour)})
    return trades
    ...
    for i in range(20, len(df)-40):
        ...
        if (row['EMA20']>row['EMA50'] and row['Swing_H']>row['Swing_L'] and row['Diff']>0):
            ...
            if gl<=row['close']<=gh and row['Bull_Sweep'] and row['low']<=row['BB_Lower']*1.02:
                ...
                trades.append({'dir':'BUY','entry':entry,'exit':exit_price,'sl':sl,'session':session})
        # SELL (Visual SL)
        if (row['EMA20']<row['EMA50'] and row['Bear_Sweep'] and row['high']>=row['BB_Upper']*0.98):
            ...
            trades.append({'dir':'SELL','entry':entry,'exit':exit_price,'sl':sl,'session':session})
    return trades

# ── 4. Equity Simulation with 1% Risk (capped) ───
def simulate_equity(trades, initial=10000, risk_pct=0.01, contract_size=10, max_contracts=10):
    equity = initial
    curve = [initial]
    result = []
    for t in trades:
        entry = t['entry']; sl = t['sl']; direction = t['dir']
        sl_dist = abs(entry - sl)
        if sl_dist < 0.5: sl_dist = 0.5          # minimum SL distance 0.5 point
        risk_amount = equity * risk_pct
        contracts = risk_amount / (sl_dist * contract_size)
        contracts = min(contracts, max_contracts) # cap to prevent insane leverage
        contracts = max(contracts, 0.01)          # at least 0.01 micro
        # PnL in points
        pnl_pts = (t['exit'] - entry) if direction=='BUY' else (entry - t['exit'])
...
print("Running v11.2...")
t_v112 = v112_trades(df_15m)
res_v112, eq_v112 = simulate_equity(t_v112)
s_v112 = stats(eq_v112, res_v112, 10000)

print("Running New V4...")
t_new = new_v4_trades(df_15m)
res_new, eq_new = simulate_equity(t_new)
s_new = stats(eq_new, res_new, 10000)

print("\n📊 POSITION SIZING (1% Risk, max 10 contracts)")
print("="*60)
print(f"{'':<25} {'v11.2':<15} {'New V4':<15}")
print(f"{'Initial':<25} ${10000:<14,.0f} ${10000:<14,.0f}")
print(f"{'Final Equity':<25} ${s_v112['final']:<14,.2f} ${s_new['final']:<14,.2f}")
print(f"{'Return':<25} {s_v112['return']:<14.2f}% {s_new['return']:<14.2f}%")
print(f"{'Max Drawdown':<25} {s_v112['dd']:<14.2f}% {s_new['dd']:<14.2f}%")
print(f"{'Win Rate':<25} {s_v112['wr']:<14.2f}% {s_new['wr']:<14.2f}%")
print(f"{'Profit Factor':<25} {s_v112['pf']:<14.2f} {s_new['pf']:<14.2f}")
print(f"{'Total Trades':<25} {s_v112['total']:<14} {s_new['total']:<14}")
