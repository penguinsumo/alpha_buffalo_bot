...
Scenario B: Sell Visual SL (BE at Mid BB) + Lower BB TP
...
# ── Sell Exit B: Visual SL (BE at Mid BB) + Lower BB TP ──
def sell_exit_visual_sl(entry, sl_initial, df15, start_idx, max_bars=40):
    pnl = 0.0
    mid_crossed = False
    for j in range(start_idx+1, min(start_idx+max_bars, len(df15))):
        r = df15.iloc[j]; h, l = r['high'], r['low']
        # เมื่อราคาลงมาแตะ Mid BB → ตั้ง Visual SL = Entry
        if not mid_crossed and l <= r['BB_Mid']:
            mid_crossed = True
            sl_initial = entry   # Visual SL = Entry
...
def run_full_backtest(df15, sell_exit_version):
    ...
    for i in range(min_bars, len(df15)-max_bars):
        ...
        if sell_exit_version == 'simple':
            ...
            pnl = sell_exit_visual_sl(entry, sl_initial, df15, i)
...
if __name__ == "__main__":
    ...
    print("🔄 Scenario B: OLD Sell with Visual SL (BE at Mid) ...")
    buy_B, sell_B = run_full_backtest(df15, 'visual_sl')
    ...
    print("\n📊 COMPARISON TABLE")
    print("="*80)
    print(f"{'':<20} {'Scenario A (Simple)':<25} {'Scenario B (Visual SL)':<25}")
