#!/usr/bin/env python3
"""
backtest_3methods.py — 3‑Layer Entry Comparison
1) OLD: HA 15m Reversal + BB (Asia Blocked)
2) NEW_V1: Golden Fibo 1H (0.5‑0.786) + Median Price + BB (Asia Rebound)
3) NEW_V2: Golden Fibo 1H (0.5‑0.786) + Sweep/VSA + BB (Asia Rebound)
"""
...
def add_indicators(df, df_1h):
    ...
    # Median Price Reversal (for NEW_V1)
    df['Median'] = (df['high'] + df['low']) / 2
    df['MP_Avg_2'] = df['Median'].rolling(2).mean().shift(1)

    # Sweep/VSA (simple) for NEW_V2
...
def compute_session_filters(df):
    ...
    # New methods: Asia Rebound – will be checked inside entry logic
    ...
    for i in range(min_bars, len(df)-max_bars):
        ...
        if row['EMA20'] > row['EMA50']:
            ...
            if method == 'OLD':
                ...
                if row['Golden_Low_1H'] and row['Golden_High_1H']:
                    in_golden = (row['Golden_Low_1H'] <= row['close'] <= row['Golden_High_1H'])
                sweep_ok = row['Bull_Sweep']  # simple sweep
...
if __name__ == "__main__":
    ...
    print("="*100)
    header = f"{'':<20} {'OLD (HA15m)':<25} {'NEW_V1 (GF0.5+MP)':<25} {'NEW_V2 (GF0.5+Sweep)':<25}"
    print(header)
    print(f"{'Trades':<20} {results['OLD'][0]['total']:<25} {results['NEW_V1'][0]['total']:<25} {results['NEW_V2'][0]['total']:<25}")
    print(f"{'Win Rate':<20} {results['OLD'][0]['wr']:.1f}%{'':<21} {results['NEW_V1'][0]['wr']:.1f}%{'':<21} {results['NEW_V2'][0]['wr']:.1f}%")
    print(f"{'Total PnL':<20} {results['OLD'][0]['pnl']:+.2f}%{'':<20} {results['NEW_V1'][0]['pnl']:+.2f}%{'':<20} {results['NEW_V2'][0]['pnl']:+.2f}%")
    print(f"{'Max DD':<20} -{results['OLD'][0]['dd']:.2f}%{'':<21} -{results['NEW_V1'][0]['dd']:.2f}%{'':<21} -{results['NEW_V2'][0]['dd']:.2f}%")

    print("\n📊 SELL SIDE COMPARISON")
    print("="*100)
    header = f"{'':<20} {'OLD (HA15m)':<25} {'NEW_V1 (GF0.5+MP)':<25} {'NEW_V2 (GF0.5+Sweep)':<25}"
    print(header)
    print(f"{'Trades':<20} {results['OLD'][1]['total']:<25} {results['NEW_V1'][1]['total']:<25} {results['NEW_V2'][1]['total']:<25}")
    print(f"{'Win Rate':<20} {results['OLD'][1]['wr']:.1f}%{'':<21} {results['NEW_V1'][1]['wr']:.1f}%{'':<21} {results['NEW_V2'][1]['wr']:.1f}%")
    print(f"{'Total PnL':<20} {results['OLD'][1]['pnl']:+.2f}%{'':<20} {results['NEW_V1'][1]['pnl']:+.2f}%{'':<20} {results['NEW_V2'][1]['pnl']:+.2f}%")
    print(f"{'Max DD':<20} -{results['OLD'][1]['dd']:.2f}%{'':<21} -{results['NEW_V1'][1]['dd']:.2f}%{'':<21} -{results['NEW_V2'][1]['dd']:.2f}%")
    print("="*100)

    # Summary total
    total_old = results['OLD'][0]['pnl'] + results['OLD'][1]['pnl']
    total_v1 = results['NEW_V1'][0]['pnl'] + results['NEW_V1'][1]['pnl']
    total_v2 = results['NEW_V2'][0]['pnl'] + results['NEW_V2'][1]['pnl']
    print(f"\n💰 Total PnL (Buy+Sell): OLD = {total_old:+.2f}% | NEW_V1 = {total_v1:+.2f}% | NEW_V2 = {total_v2:+.2f}%")
