#!/usr/bin/env python3
"""
backtest_real_fixed.py — ใช้ GC=F (Gold Futures) จาก yfinance
เปรียบเทียบ Original v11.2 vs New V4 บนข้อมูลจริง 90 วัน
"""
...
def load_real_data(days=90):
    # ลอง yfinance ด้วย GC=F
    try:
        import yfinance as yf
        end = datetime.now(timezone.utc)
        start = end - timedelta(days=days)
        ticker = "GC=F"          # <-- เปลี่ยนจาก XAUUSD=X
        ...
        logger.info(f"✅ yfinance (GC=F): {len(df)} candles")
        return df
...
if __name__ == "__main__":
    days = 90
    logger.info(f"กำลังโหลดข้อมูลจริง GC=F {days} วัน...")
    ...
    print("\n" + "="*80)
    print("📊 เปรียบเทียบ Original v11.2 (ซ้าย) vs New V4 (ขวา) — GC=F")
    print("="*80)
