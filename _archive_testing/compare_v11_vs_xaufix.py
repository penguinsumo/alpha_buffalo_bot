...
# ─── 1. โหลดข้อมูล + สร้าง datetime index ──────────
df = pd.read_csv('data/XAUUSD_H1.csv', index_col=0)
df = df[['open','high','low','close']].rename(columns=str.lower)
df.index = pd.date_range(start='2026-01-01', periods=len(df), freq='h')
df = df.sort_index()

START = '2026-03-02'
END   = '2026-06-15'
df_period = df.loc[START:END].copy()
print(f"📅 Data: {START} → {END} | Candles: {len(df_period)}")
if len(df_period) == 0:
    print("❌ Still no candles. Check if date range is within data.")
    exit()

# ─── 2. Indicators ─────────────────────────────────
df_period['ema20'] = df_period['close'].ewm(span=20).mean()
df_period['ema50'] = df_period['close'].ewm(span=50).mean()
...
configs = [
    ("1. XAU FIX v1 (TP/SL ตายตัว)", df_period['v5_buy'], df_period['v5_sell'], 'fixed', df_period['v5_buy'], df_period['v5_sell']),
...
# ─── 5. Display ────────────────────────────────────
print("\n" + "="*85)
print("📊 เปรียบเทียบ XAU FIX v1 กับ v11.2 บนข้อมูลเดียวกัน (2 Mar - 15 Jun 2026 H1)")
print("="*85)
