...
# ─── 1. ดึงข้อมูลจาก Twelve Data API ──────────────
API_KEY = "4b75c8722e2a4af4913561a2eeaf37ac"  # จาก HANDOVER (ถ้าเปลี่ยนให้แก้ที่นี่)
SYMBOL = "XAU/USD"
INTERVAL = "1h"
...
if "values" not in resp:
    ...
    exit()
data = pd.DataFrame(resp["values"])
data = data[['datetime','open','high','low','close']].rename(columns={'datetime':'date'})
...
df_period = data.copy()
print(f"✅ Loaded {len(df_period)} candles from {df_period.index[0]} to {df_period.index[-1]}")
