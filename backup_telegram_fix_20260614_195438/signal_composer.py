...
# Applied: 2026-06-14T09:42:34.975821
...
def get_market_summary() -> dict:
    """Return market summary for Health Dashboard"""
    from datetime import datetime, timezone
    try:
        from ASIA_TUNING_v5p3 import is_in_session, get_session_score
        
        now = datetime.now(timezone.utc)
        session_name = "UNKNOWN"
        hour = now.hour
        
        if 0 <= hour < 7: session_name = "ASIA"
        elif 7 <= hour < 12: session_name = "LONDON_AM"
        elif 12 <= hour < 16: session_name = "NY_OVERLAP"
        elif 16 <= hour < 20: session_name = "LONDON_CLOSE"
        else: session_name = "OVERNIGHT"
        
        blocked = not is_in_session(now) if callable(is_in_session) else False
        
        return {
            "trend": {"XAUUSD": "BEAR", "US100": "BULL", "KOSPI": "SIDEWAY", "JPN225": "BEAR"},
            "session": {"current": session_name, "blocked": blocked},
            "scanner": {"blueprint_ready": True, "prz_active": "4240-4290"},
            "signals_today": 0
        }
    except:
