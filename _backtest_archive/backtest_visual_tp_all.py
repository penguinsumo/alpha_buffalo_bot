#!/usr/bin/env python3
"""Visual TP Backtest ทุก Session (Twelve Data 15m)"""
...
# ── 3. Visual TP Trade Logic (ทั้ง Buy/Sell) ──
...
# ── 5. Run ──
trades = generate_trades(df_15m)
stats = simulate_per_session(trades)

print("\n📊 VISUAL TP — ALL SESSIONS (Twelve Data 15m, 60 days)")
print("="*80)
print(f"{'Session':<10} {'Trades':<8} {'Win Rate':<10} {'Return':<12} {'Max DD':<10} {'Profit Factor':<15}")
print("-"*80)
for sess in ['ASIA', 'LONDON', 'NY']:
    s = stats.get(sess)
    if s:
        print(f"{sess:<10} {s['trades']:<8} {s['wr']:<10.2f}% {s['return']:<12.2f}% {s['dd']:<10.2f}% {s['pf']:<15.2f}")
print("="*80)
