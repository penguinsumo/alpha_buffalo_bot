...
for i in range(20, len(df)-40):
    ...
    if signal:
        ...
        trades.append({'time': ts, 'session': signal['session'], 'dir': 'SELL',
                       ...
                       'be_policy': signal.get('be_policy'),
                       'trail_policy': signal.get('trail_policy'),
                       'v5_quality_score': signal.get('v5_quality_score'),
                       'v5_quality_grade': signal.get('v5_quality_grade'),
                       'v5_basis': signal.get('v5_basis'),
                       'v5_premium_any': signal.get('v5_premium_any', False),
                       'v5_premium_micro_bos': signal.get('v5_premium_micro_bos', False),
                       'v5_premium_reclaim': signal.get('v5_premium_reclaim', False),
                       'v5_premium_sweep_ha': signal.get('v5_premium_sweep_ha', False),
                       'close_below_ema20': signal.get('close_below_ema20', False),
                       'close_below_bb_mid': signal.get('close_below_bb_mid', False),
                       'ema20_below_ema50': signal.get('ema20_below_ema50', False),
                       'bb_upper_touch_strength': signal.get('bb_upper_touch_strength'),
                       'sell_rejection_wick_ratio': signal.get('sell_rejection_wick_ratio'),
                       'candle_body_ratio': signal.get('candle_body_ratio'),
                       'atr14_at_entry': signal.get('atr14_at_entry'),
                       'entry_to_sl_points': signal.get('entry_to_sl_points'),
                       'entry_to_tp_points': signal.get('entry_to_tp_points'),
                       'entry_rr': signal.get('entry_rr'),
                       'session_quality_gate': signal.get('session_quality_gate'),
                       'sell_dot_reason': signal.get('sell_dot_reason'),
