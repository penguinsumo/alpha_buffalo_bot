#!/usr/bin/env python3
"""เปรียบเทียบ LONDON Sell: Original New V4 (Visual SL + BE 0.15%) vs Trailing Sell + BE 0.10%"""
import requests, pandas as pd, numpy as np, os, warnings
from datetime import datetime, timedelta
from collections import defaultdict
warnings.filterwarnings('ignore')

# ── 1. โหลดข้อมูล Twelve Data 15m ──
...
# ── 3. Trade Logic (A: Original New V4, B: Trailing Sell + BE 0.10%) ──
def generate_trades(df, method='A'):
    trades = []
    for i in range(20, len(df)-40):
        row = df.iloc[i]; ts = row.name
        if not (1 <= ts.hour < 19): continue
        # BUY (เหมือนกันทั้งสองวิธี)
        ...
        if (row['EMA20'] < row['EMA50'] and row['Bear_Sweep'] and row['High'] >= row['BB_Upper']*0.98):
            ...
            if method == 'A':
                # Original: Visual SL (แตะ Mid → SL = Entry), BE 0.15%
                mid_crossed=False
                for j in range(i+1, min(i+40, len(df))):
                    r = df.iloc[j]; h, l = r['High'], r['Low']
                    ...
                    exit_price = df.iloc[min(i+40-1, len(df)-1)]['Close']
            else:  # B: Trailing Sell + BE 0.10%
                lowest = entry; be_act=False
                for j in range(i+1, min(i+40, len(df))):
                    r = df.iloc[j]; h, l = r['High'], r['Low']
                    if l < lowest: lowest = l
                    if not be_act and lowest <= entry * 0.9990:   # BE 0.10%
                        be_act=True; sl=entry
                    if be_act:
                        sl = min(sl, lowest * 1.0005)   # Trailing 0.05%
                    if l <= r['BB_Lower']: exit_price=r['BB_Lower']; break
                    if h >= sl: exit_price=sl; break
                else:
                    exit_price = df.iloc[min(i+40-1, len(df)-1)]['Close']
...
# ── 4. Simulation per session (same as before) ──
...
# ── 5. Run ──
trades_A = generate_trades(df_15m, 'A')
trades_B = generate_trades(df_15m, 'B')

stats_A = simulate_per_session(trades_A)
stats_B = simulate_per_session(trades_B)

print("\n📊 LONDON COMPARISON: Original (Visual SL + BE 0.15%) vs Trailing (BE 0.10%)")
print("="*80)
print(f"{'Metric':<20} {'A: Visual SL':<25} {'B: Trailing Sell':<25}")
print(f"{'Trades':<20} {stats_A['LONDON']['trades']:<25} {stats_B['LONDON']['trades']:<25}")
print(f"{'Win Rate':<20} {stats_A['LONDON']['wr']:.2f}%{'':<19} {stats_B['LONDON']['wr']:.2f}%")
print(f"{'Return':<20} {stats_A['LONDON']['return']:.2f}%{'':<19} {stats_B['LONDON']['return']:.2f}%")
print(f"{'Max DD':<20} {stats_A['LONDON']['dd']:.2f}%{'':<19} {stats_B['LONDON']['dd']:.2f}%")
print(f"{'Profit Factor':<20} {stats_A['LONDON']['pf']:.2f}{'':<19} {stats_B['LONDON']['pf']:.2f}")
print(f"{'Days Stopped':<20} {stats_A['LONDON']['stopped']:<25} {stats_B['LONDON']['stopped']:<25}")
