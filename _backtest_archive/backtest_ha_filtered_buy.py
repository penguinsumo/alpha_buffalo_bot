#!/usr/bin/env python3
"""
Baseline (Buy NY>=15) + HA-Filtered Buy in ASIA/LONDON Profit Hours (BKK)
HA Condition: Heikin-Ashi candle ต้องเป็นขาขึ้น (HA_Close > HA_Open) ในแท่ง 15m
"""
import sys, os
...
def add_indicators(df):
    ...
    # Recursive but approximate (we'll use a proper computation below)
    # Proper HA calculation
...
# ====== 2. GATE (HA-Filtered Buy for selected hours) ======
...
class HAFilteredBuyGate:
    ...
    def evaluate(self, session_state: SessionState, direction: str, df=None, idx=None,
        ...
        if direction == 'BUY':
            ...
            if sess == 'ASIA' and bkk_hour in self.ASIA_BUY_HOURS_BKK:
                # Check HA filter if df, idx provided
                if df is not None and idx is not None:
                    row = df.iloc[idx]
                    if not row['HA_Bullish']:
...
gate_ha = HAFilteredBuyGate(clock)
buy_eng = BuySignalEngine()
sell_eng = SellSignalEngine()

def generate_trades(gate, use_ha=False):
    trades = []
    for i in range(20, len(df)-40):
        ...
        # BUY
        gate_buy = gate.evaluate(session_state, 'BUY', df=df, idx=i, daily_dd_ok=True, consec_loss_ok=True) if use_ha else gate.evaluate(session_state, 'BUY', daily_dd_ok=True, consec_loss_ok=True)
...
# Baseline gate (NY only)
from engine_v4.session_gate import SessionGate as OrigGate
gate_base = OrigGate(clock)
trades_base = generate_trades(gate_base)
trades_ha = generate_trades(gate_ha, use_ha=True)
print(f"Baseline Trades: {len(trades_base)}")
print(f"HA-Filtered Trades: {len(trades_ha)}")

# Simulate
def simulate(trades, initial=10000, risk_pct=0.0075, max_contracts=10, daily_dd_limit=0.03, max_consec_loss=5):
    trades = sorted(trades, key=lambda x: x['time'])
    sessions = defaultdict(lambda: {'trades':[], 'curve':[initial], 'equity':initial, 'daily_eq_start':initial, 'current_day':None, 'consec_loss':0, 'stop_day':False, 'stopped':0, 'max_dd':0})
    for t in trades:
        sess = t['session']; sd = sessions[sess]
        day = t['time'].date()
        if day != sd['current_day']:
            sd['current_day'] = day; sd['daily_eq_start'] = sd['equity']; sd['consec_loss'] = 0; sd['stop_day'] = False
    ...
    for sess, sd in sessions.items():
        ...
        stats[sess] = {'trades':len(t_list),'wr':wr,'return':ret,'dd':sd['max_dd'],'pf':pf,'stopped':sd['stopped'],'final_eq':final_eq}
    return stats

stats_base = simulate(trades_base)
stats_ha = simulate(trades_ha)
...
print("\n" + "="*90)
print(f"{'HA-Filtered Buy (ASIA/LONDON Profit Hours) vs BASELINE':^90}")
print("="*90)
for sess in ['ASIA','LONDON','NY']:
    h = stats_ha.get(sess, {})
    b = stats_base.get(sess, {})
    print(f"\n-- {sess} --")
    print(f"{'Metric':<18}{'HA-Filtered':<22}{'Baseline':<22}{'Diff':<12}")
    for key, label, kind in [('trades','Trades','int'),('wr','Win Rate','pct'),('return','Return','pct'),('dd','Max DD','pct'),('pf','Profit Factor','pf')]:
        hv = h.get(key,0); bv = b.get(key,0)
        diff = hv - bv if kind in ('pct','pf','int') else ''
        print(f"{label:<18}{fmt(hv,kind):<22}{fmt(bv,kind):<22}{diff if diff != '' else '':<12}")

tot_ha = sum(v['return'] for v in stats_ha.values())
tot_base = sum(v['return'] for v in stats_base.values())
print(f"\nTOTAL: HA-Filtered = {fmt(tot_ha,'pct')}, Baseline = {fmt(tot_base,'pct')}")
