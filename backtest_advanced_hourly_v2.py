#!/usr/bin/env python3
"""Advanced Hourly Breakdown v2 — Rolling Consistency Score (fixed)"""
...
# ── 3. Rolling Consistency Score (Fixed) ──
def rolling_consistency(pnl_series, window=20, min_samples=10):
...
for h in range(1,19):
    ...
    for d in ['BUY','SELL']:
        sub_d = sub[sub['dir']==d].sort_values('time') if 'time' in sub.columns else sub[sub['dir']==d]
        if len(sub_d) < 10: continue
        pnl_series = sub_d['pnl']
        avg_wr, cons = rolling_consistency(pnl_series, window=20, min_samples=10)
        ...
        results.append({'hour':h, 'dir':d, 'trades':len(sub_d), 'wr':wr, 'pnl_sum':pnl_sum,
                        'avg_vol':avg_vol, 'avg_rolling_wr':avg_wr*100, 'consistency':cons,
                        'session':sub_d['session'].iloc[0]})

# ── 4. Print Enhanced Table ──
print("\n📊 ENHANCED HOURLY BREAKDOWN (Rolling WR, Consistency)")
print("="*110)
print(f"{'Hour':<6} {'Dir':<6} {'Trades':<8} {'WR':<8} {'PnL Sum':<10} {'Avg Vol':<10} {'Roll WR':<10} {'Consistency':<12} {'Session':<10}")
for r in sorted(results, key=lambda x: x['consistency'], reverse=True):
    print(f"{r['hour']:02d}:00  {r['dir']:<6} {r['trades']:<8} {r['wr']:<8.1f}% {r['pnl_sum']:<10.1f} {r['avg_vol']:<10.2f} {r['avg_rolling_wr']:<10.1f}% {r['consistency']:<12.3f} {r['session']:<10}")
...
for ses in ['ASIA','LONDON','NY']:
    ...
    for d in ['BUY','SELL']:
        ...
        for rank, r in enumerate(top2, 1):
            print(f"  {d} #{rank}: UTC {r['hour']:02d}:00 | Trades:{r['trades']} WR:{r['wr']:.1f}% PnL:{r['pnl_sum']:+.1f} RollingWR:{r['avg_rolling_wr']:.1f}% Consistency:{r['consistency']:.3f}")
