//+------------------------------------------------------------------+
//| AlphaBuffalo_RailwayPythonEA_v100.mq5                     |
//| Railway Python engine -> isolated MT5 execution only                |
//+------------------------------------------------------------------+
#property strict
...
input string ApiSymbol          = "XAUUSD"; // canonical server symbol; never broker suffix
input string ClientId           = "RAILWAY_PYTHON_V1";
input int    PollSeconds        = 15;
input double BaseLot            = 0.01;
input int    Magic              = 20260719; // isolated from every legacy AlphaBuffalo EA
...
input bool   RequireHedgingAccount = true; // protects other EAs on the same symbol
input double DailyProfitTargetPct = 8.0; // 0 = server-only/disabled locally
...
{
   return "ABR.DSE." + (string)AccountInfoInteger(ACCOUNT_LOGIN) + "." + (string)Magic + "." + BangkokDayKey();
}

string DailyProfitLockVariable()
{
   return "ABR.DPL." + (string)AccountInfoInteger(ACCOUNT_LOGIN) + "." + (string)Magic + "." + BangkokDayKey();
}
...
int OnInit()
{
   ...
   if(RequireHedgingAccount && margin_mode != ACCOUNT_MARGIN_MODE_RETAIL_HEDGING)
   {
      Print("AlphaBuffalo Railway EA stopped: a hedging account is required to isolate it from other EAs on the same symbol");
      return INIT_FAILED;
   }
   if(CanonicalSymbol(ApiSymbol) != CanonicalSymbol(_Symbol))
   {
      Print("AlphaBuffalo Railway EA stopped: ApiSymbol=", ApiSymbol,
            " does not match broker chart symbol=", _Symbol);
   ...
   DayStartEquity();
   Print("AlphaBuffalo Railway Python EA v1.00 started | API=", ApiUrl,
         " | broker_symbol=", _Symbol, " | api_symbol=", ApiSymbol,
         " | Magic=", Magic,
         " | PollSeconds=", seconds);
...
void PollAndExecute()
{
   ...
   string url = ApiBase() + "/execution/python/command?key=" + LicenseKey +
                "&symbol=" + ApiSymbol +
                "&client_id=" + ClientId +
   ...
   if(payload_symbol != "" &&
      CanonicalSymbol(payload_symbol) != CanonicalSymbol(ApiSymbol))
   {
...
void ExecuteOpen(string command, string command_id)
{
   ...
   string comment = "ABR|" + StringSubstr(signal_id, 0, 23);
   bool sent = false;
   if(direction == "BUY")
