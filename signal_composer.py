...
return SignalDecision(status="NO_SIGNAL", reason="no_direction", score=base_score)

        # 3. Session-Hour Permission Gate
        if session == 'CLOSED':
            debug_stage('DECISION_BUILD')
return SignalDecision(status="NO_SIGNAL", reason="session_closed", score=base_score)
...
return SignalDecision(status="NO_SIGNAL", reason="below_threshold",
        ...
        if session == "ASIA":
            if not ASIAScalpTriggerGate.verify_sweep(micro_sigs):
                debug_stage('DECISION_BUILD')
return SignalDecision(status="NO_SIGNAL", reason="asia_sweep_fail", score=base_score)
            if not ASITuningManager.is_within_safe_time(h4_info["current_hour_utc"]):
                debug_stage('DECISION_BUILD')
return SignalDecision(status="NO_SIGNAL", reason="asia_safe_time_fail", score=base_score)

        # 6. Basket Layer
        basket = self.buy_basket if best_dir == "BUY" else self.sell_basket
        if basket.killed:
            debug_stage('DECISION_BUILD')
return SignalDecision(status="NO_SIGNAL", reason="basket_killed", score=base_score)
        layer = basket.layer + 1
        lot_multi = COMPOSER_CONFIG[f"lot_layer_{min(layer, 2)}"]
        if layer > 2:
            debug_stage('DECISION_BUILD')
return SignalDecision(status="NO_SIGNAL", reason="max_layer_reached", score=base_score)
