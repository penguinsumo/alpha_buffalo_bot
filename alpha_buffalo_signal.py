...
SYMBOL              = "XAU/USD"
COOLDOWN_MIN        = 10
POLL_INTERVAL       = 120  # signal loop (วินาที)
CMD_INTERVAL        = 3    # command loop (วินาที)
...
def load_last_signal():
        """โหลด last_signal จาก Supabase เมื่อ restart"""
...
def get_user_role(user_id: int) -> str:
        if not supabase:
                ...
                try:
                            ...
                            log(f"get_user_role error: {e} — fallback hardcode")
                            return "admin" if user_id == ADMIN_ID else "guest"
                        return "guest"
...
def handle_commands():
    ...
    for update in get_updates():
        ...
        # /help
            ...
            role = get_user_role(user_id)
            if role == "guest":
                                send_message(chat_id, "⛔ ไม่มีสิทธิ์ใช้งาน")
elif role == "member":
                send_message(chat_id,
                            ...
                            if not arg:
                                            ...
                                            try:
                                                                ...
                                                                if add_user(tid, name, "member", user_id):
                                                                                        ...
                                                                                        send_message(tid, "✅ คุณได้รับสิทธิ์ใช้งาน Alpha Buffalo Bot แล้ว!")
                                            else:
